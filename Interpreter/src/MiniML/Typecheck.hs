...
-- Let rec
typeCheck env (LetRec _ f x tx tf e1 e2) = do 
  let env' = M.insert x tx env
  let env'' = M.insert f (TArrow tx tf) env'
  t1 <- typeCheck env'' e1 
  if t1 == tf then typeCheck env'' e2
  else typeError (getPosn e1) ("Expression is expected to have type "<> showType tf <> " but has type "<>showType t1)
-- References
typeCheck env (Asgn _ e1 e2) = do 
  ...
  case t1 of 
    TRef rt -> do
      t2 <- typeCheck env e2 
      if t2 == rt then return TUnit -- asgn was successful but has no type
      else typeError (getPosn e2) ("Expression is expected to have type " <> showType rt <> " but has type " <> showType t2)
    _ -> typeError (getPosn e1) ("Expression is expected to have type TRef * , but has type " <> showType t1)
typeCheck env (Deref _ ptr) = do 
  p <- typeCheck env ptr
  case p of 
    TRef t -> return t 
    _ -> typeError (getPosn ptr) ("Expression is expected to have type TRef * but has type " <> showType p)  
